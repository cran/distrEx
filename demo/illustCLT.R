...
illustrateCLT(Distr = Exp(), len = 20)

N <- Norm(mean = 2, sd = 1.3)
P <- Pois(lambda = 1.2)
Z <- 2 * N + 3 + P # exact transformation
illustrateCLT(Distr = sin(abs(Z)), len = 20) #something weird
...
illustrateCLT(Distr = Binom(size = 5), len = 20)
illustrateCLT(Distr = Nbinom(size = 5), len = 20)
